...
**v3 candidate revision, 2026-09-05 (SM 1217501628088122).** This copy narrows §9.3's first power
qualification with the elected-annual-test-date mechanism (ASC 350-20-35-28) and the probe's
measured modal-quarter shares (`docs/scouting/probes/annual_test_date.py`); everything else is
identical to `docs/papers-v2/paper-III-dual-tensor/paper-III.md`. Whether this revision ships,
supersedes v2, or is discarded is Jason's call — v2 remains the paper he reads and is untouched by
this copy.
...
observed lag distribution and adds a noiseless per-tier shift), and that assumption is not free: the
onset carries a measurable audit-calendar signature. ASC 350-20-35-28 lets a firm elect any annual
goodwill-test date, bolting a uniform per-firm delay onto every lag regardless of tier, and a probe
built to detect exactly that (`docs/scouting/probes/annual_test_date.py`, scoped to the instrument's
own calendar behaviour and not a tier retest — PRE-002 §5's stopping rule forbids a third instrument
on this data) confirms one: charges concentrate in a firm's own modal fiscal quarter at 0.72–0.75
(pilot) and 0.58–0.67 (replication) against a ~0.49–0.60 null at every firm-count threshold, *p* <
0.0001 throughout. So any measurement error in the onset attenuates a true gradient, and the true
power is lower by an amount this measures rather than one left unquantified. The 688 events
