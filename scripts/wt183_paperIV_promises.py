...
ROWS = [
 ...
 ("paper-IV", "50848d3974", "src/wealth_tensor/excess_demand.py", "H",
  "grep -on 'src/wealth_tensor/[a-z_]*\\.py' docs/papers/paper-IV-composition/paper-IV.md",
  "prints exactly four: L684 redistribution.py and L685 lag.py and L686 lambda_sensitivity.py, all named in the same bullet as Paper II's and Paper III's, and L700 excess_demand.py. So excess_demand.py is still this paper's only src/ dependency that is not a sibling's, and wt182's change of 'under both' to 'under all three' does not touch that claim",
  ),
